...
func _unhandled_input(event: InputEvent) -> void:
	# Perform mouse look. Gamepad look is performed in `_process()` instead.
...
func _process(delta: float) -> void:
	# Perform gamepad look. Mouse look is performed in `_unhandled_input()` instead.
	var gamepad_look := Input.get_vector(&"look_left", &"look_right", &"look_up", &"look_down")
	_yaw -= gamepad_look.x * GAMEPAD_SENSITIVITY * 0.5 * delta
	_pitch += gamepad_look.y * GAMEPAD_SENSITIVITY * delta
	_pitch = clamp(_pitch, -PI, PI)
	$Rig.rotation = Vector3(0, _yaw, 0)
