...
signal note_hit(hit_type: Enums.HitType, hit_error: float)
...
func _ready() -> void:
	...
	var chart_data: Array[Array]
	match chart:
		ChartData.Chart.THE_COMEBACK:
			chart_data = ChartData.THE_COMEBACK_DATA
		ChartData.Chart.SYNC_TEST:
			chart_data = ChartData.SYNC_TEST_DATA
		_:
			assert(false, "Unknown chart: %d")
			chart_data = ChartData.THE_COMEBACK_DATA
...
func _miss_old_notes() -> void:
	while not _notes.is_empty():
		...
		if note_delta > HIT_MARGIN_GOOD:
			...
			note_hit.emit(Enums.HitType.MISS_LATE, note_delta)
		else:
...
func _handle_keypress() -> void:
	...
	if hit_delta < -HIT_MARGIN_MISS:
		...
		note_hit.emit(Enums.HitType.PERFECT, hit_delta)
		...
		_play_stats.mean_hit_error = _hit_error_acc / _hit_count
		if hit_delta < 0:
			note_hit.emit(Enums.HitType.GOOD_EARLY, hit_delta)
		else:
			note_hit.emit(Enums.HitType.GOOD_LATE, hit_delta)
		...
		_play_stats.mean_hit_error = _hit_error_acc / _hit_count
		if hit_delta < 0:
			note_hit.emit(Enums.HitType.MISS_EARLY, hit_delta)
		else:
			note_hit.emit(Enums.HitType.MISS_LATE, hit_delta)
